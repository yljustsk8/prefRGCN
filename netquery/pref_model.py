...
class PrefRGCN(nn.Module):
    ...
    def __init__(self, gq_model, gq_dec_type, emb_dim, hidden, rel_num=4):
        ...
        self.cos = nn.CosineSimilarity(dim=0) # 评价函数 改成合适的

    def forward(self, formula, prefs, edge_index, edge_type, batch, targets):
        '''
    ...
    def margin_loss(self, formula, prefs, edge_index, edge_type, batch, margin=1):
        ...
        pos_scores = self.forward(formula, prefs, edge_index, edge_type, batch, targets=pos_nodes)
        neg_scores = self.forward(formula, prefs, edge_index, edge_type, batch, targets=neg_nodes)
